 ...
 */
const DATASET_PATH = "https://f32-pbi.f32prod.mfg.intel.com/reports/api/v2.0/datasets(path='/F32_FAB/SharedTechnologies/WetEtch/PAM/DATASETS/";
...
function checkAndQuery(){
    ...
    try {
        ...
        ceidList.length === 0 ? console.log('0 CEIDs selected. Waiting to query.') : queryData()
    } catch (e) {
        console.log('0 CEIDs selected. Waiting to query.')
    }
}


function queryData() {
    console.log('Data would be querying if code gets here.')
}
...
function loadAndFilterDataset(datasetName, paramName, paramValue, callback) {
    ...
    paramName ? url = `${url}&${paramName}=${paramValue}` : console.log('parameter not present')
...
function pamTable(entity) {
    ...
    workOrderData = DATASETS.WORKORDERS
    ...
    function buildSPCSection() {
        ...
        SPCdata = dataEquals(DATASETS.SPC, 'ENTITY', entity)
