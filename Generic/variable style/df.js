 ...
 * @returns {*}
 */
let dataEquals = (data, key, val, dataType) => {
  let results = []
 ...
 * @returns
 */
let dataNotEquals = (data, key, val) => {
  let results = []
  data.forEach(row => {
 ...
 * @returns {Dict|Map}
 */
let dataNotLike = (data, key, value) => {
  let results = []
  data.forEach(row => {
 ...
 * the let  only needs to reference data instead of data.value
